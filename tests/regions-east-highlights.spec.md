# QA Checklist: Region Highlights Feature (East India)

## 1. Page Rendering
- [ ] Navigate to `/regions/east-india`
- [ ] **Data Fetching**: Verify the page loads without crashing.
- [ ] **Hero Section**:
    - [ ] Hero Image is visible (check `east-india-hero.jpg` placeholder or real URL).
    - [ ] Title "East India" is H1.
...
- [ ] **Lightbox Open**: Click on any thumbnail (e.g., Konark Sunrise). Dialog/Modal should open.
- [ ] **Image Rendering**: The full-size image is displayed in the modal.
- [ ] **Caption**: Caption text "Konark Sun Temple at dawn" is visible.
- [ ] **Navigation**:
...
- [ ] **Open Section**: Verified `region_highlights_open` fires when the section scrolls into view (Intersection Observer).
    - *Check console logs.*
- [ ] **Click**: Verified `region_highlight_click` fires with correct `highlight_id` (e.g., `konark-sunrise`) when a thumbnail is clicked.
- [ ] **Share**: Verified `region_highlight_share` fires when Share button is clicked.

## 4. Accessibility
- [ ] Images have valid `alt` text (e.g., "Sunrise at Konark Sun Temple with sculpted wheels").
