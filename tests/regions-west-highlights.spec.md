# QA Checklist: Region Highlights Feature (West India)

## 1. Page Rendering
- [ ] Navigate to `/regions/west-india`
- [ ] **Data Fetching**: Verify the page loads without crashing.
- [ ] **Hero Section**:
    - [ ] Hero Image is visible (check `west-india-hero.jpg` placeholder or real URL).
    - [ ] Title "West India" is H1.
...
- [ ] **Lightbox Open**: Click on any thumbnail (e.g., Jaisalmer Dunes). Dialog/Modal should open.
- [ ] **Image Rendering**: The full-size image is displayed in the modal.
- [ ] **Caption**: Caption text "Dunes near Jaisalmer" is visible.
- [ ] **Navigation**:
    - [ ] Click "Right Arrow" button -> Next image (Goa Beach).
...
- [ ] **Open Section**: Verified `region_highlights_open` fires when the section scrolls into view (Intersection Observer).
    - *Check console logs.*
- [ ] **Click**: Verified `region_highlight_click` fires with correct `highlight_id` (e.g., `jaisalmer-dunes`) when a thumbnail is clicked.
- [ ] **Share**: Verified `region_highlight_share` fires when Share button is clicked.

## 4. Accessibility
- [ ] Images have valid `alt` text (e.g., "Sand dunes near Jaisalmer at sunset").
