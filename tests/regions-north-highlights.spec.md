...
# QA Checklist: Region Highlights Feature

## 1. Page Rendering
- [ ] Navigate to `/regions/north-india`
- [ ] **Data Fetching**: Verify the page loads without crashing.
- [ ] **Hero Section**:
    - [ ] Hero Image is visible (check `north-india-hero.jpg` placeholder or real URL).
    - [ ] Title "North India" is H1.
    - [ ] Intro text is readable.
- [ ] **Highlights Section**:
    - [ ] Section title "Visual Highlights" is present.
    - [ ] Grid displays exactly 6 thumbnails initially (if data has >6).
    - [ ] "View all" button appears if more than 6 items exist.

## 2. Interaction & Lightbox
- [ ] **Lightbox Open**: Click on any thumbnail. Dialog/Modal should open.
- [ ] **Image Rendering**: The full-size image is displayed in the modal.
- [ ] **Caption**: Caption text and Credit are visible at the bottom.
- [ ] **Navigation**:
    - [ ] Click "Right Arrow" button -> Next image.
...
- [ ] **Open Section**: Verified `region_highlights_open` fires when the section scrolls into view (Intersection Observer).
    - *Check console logs if `trackEvent` logs there.*
- [ ] **Click**: Verified `region_highlight_click` fires with correct `highlight_id` when a thumbnail is clicked.
- [ ] **Share**: Verified `region_highlight_share` fires when Share button is clicked.

## 4. Accessibility
- [ ] Images have valid `alt` text.
...
- [ ] No layout shift when lightbox opens (scroll lock).
